...
RSpec.describe EmailConfirmationForSignerEmailJob, type: :job do
  ...
  let(:parish) { FactoryBot.create(:parish, :london_and_westminster) }
  ...
  it "sets the parish_id" do
    expect(Parish).to receive(:find_by_postcode).with("SW1A1AA").and_return(parish)

    expect{ run_jobs_and_reload_signature }.to change{ signature.reload.parish_id }.from(nil).to("3415")
  end
end
